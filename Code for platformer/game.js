//Add relation between the symbol and the type.
var actorChars = {
	'@': Player,
	'o': Coin,
...
//Whenever you write a constructor for an actor, make sure you are giving it a type or none of its functions will work.
function Coin(pos) {
  ...
  while (step > 0) {
    var thisStep = Math.min(step, maxStep);
		this.actors.forEach(function(actor) {
			//Any interactive object has to have an act function and that act function has to be defined. this information below.
			//First thing I need is thisStep for always starting off.
...
Level.prototype.playerTouched = function(type, actor) {
	if (type == 'coin') {
		this.acotrs = this.actors.filter(function(other) {
			return other != actor;
		});
	}
};
